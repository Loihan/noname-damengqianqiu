// 雷殛边框特效（⚡电光/💥破字）已抽离至 effect/sgz_machao.js
import { initLeijiThunder, removeLeijiThunder, initLeijiBoom, removeLeijiBoom } from '../effect/sgz_machao.js';
...
export default {
    ...
    skills: {
        ...
        sgz_leiji: {
            ...
            // === 响应【闪】时的目标选择：优先敌人，且敌人中优先高威胁度目标 ===
            ai2: function(target) {
                var evt = _status.event;
                var player = evt.player;
                var needSha = evt.filterCard({ name: 'sha' }, player, evt);
                var needShan = evt.filterCard({ name: 'shan' }, player, evt);
                if (needShan && !needSha) {
                    // 打出【闪】：敌人基础分 100 + 威胁度加成（get.threaten 基于目标技能评估）
                    var att = get.attitude(player, target);
                    if (att < 0) {
                        return 100 + get.threaten(target, player) * 10;
                    }
                    // 非敌人（自己/队友）低分兜底，保证没有敌人可选时仍能打出闪
                    return Math.max(1, att);
                }
                // 出牌阶段使用【杀】等：保持原有评估逻辑不变（与引擎默认 ai2 等价）
                if (evt.name == 'chooseToUse') {
                    return get.effect_use(target);
                }
                return 1;
            },
            onuse: function(result, player) {
                ...
                if (horses.length > 0) {
                    ...
                    if (target != player) {
                        ...
                        // 「破」标记全场共享：所有角色（除目标本人外）对其用牌无距离/次数限制
                        game.players.forEach(function(p) {
                            p.addSkill('sgz_leiji_boom_mod');
                        });
            ...
            sgz_leiji_effect: {
                ...
                onremove: function(player) {
                    ...
                    // 清理全场共享的「破」标记效果（挂在所有角色身上的 mod）
                    game.players.forEach(function(p) {
                        if (p.hasSkill('sgz_leiji_boom_mod')) {
                            p.removeSkill('sgz_leiji_boom_mod');
                        }
                    });
                },
            },
            sgz_leiji_thunder: {
                ...
                // 边框特效：外层电光缠绕（实现见 effect/sgz_machao.js）
                init: initLeijiThunder,
                onremove: removeLeijiThunder,
            },
            sgz_leiji_boom: {
                charlotte: true,
                mark: true,
                ...
                intro: {
                    name: "雷殛·破",
                    content: "除你本人外，所有角色对你使用牌无距离和次数限制"
                },
                // 边框特效：四个碎裂"破"字令符（实现见 effect/sgz_machao.js）
                init: initLeijiBoom,
                onremove: removeLeijiBoom,
            },
            // 「破」标记的全局效果：全场所有角色（除目标本人外）对带💥标记的角色使用牌无距离/次数限制。
            // 挂在所有角色身上（框架 checkMod 只遍历用牌者的 mod，故必须给每人挂一份），
            // 由 sgz_leiji_effect 在马超回合结束时统一移除。
            sgz_leiji_boom_mod: {
                charlotte: true,
                mod: {
                    targetInRange: function(card, player, target) {
                        if (player != target && target.hasSkill('sgz_leiji_boom')) {
                            return true;
                        }
                    },
    ...
    skillTranslate: {
        ...
        sgz_leiji: "雷殛",
        sgz_leiji_info: "当你需要使用或打出【杀】/【闪】时，你可以弃置一名角色装备区的第一张进攻/防御坐骑牌，视为你使用或打出了一张【杀】/【闪】（依此法使用的杀不计入次数且目标始终为依此法失去坐骑牌的角色）。依此法失去坐骑牌的其他角色：<br>①此牌结算后：若其有已废除的武器栏/防具栏，恢复之。<br>②直到其回合结束：受到的伤害+1且改为雷电伤害。<br>③直到你的回合结束：除其以外所有角色对其使用牌无距离次数限制。",
        sgz_mingzong:"鸣踪",
