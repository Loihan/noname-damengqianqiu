export default {
    character: {
        ...
        sgz_zhaoyun: {
            ...
            hp: 1, 
            maxHp: 2,
            skills: ["sgz_juejing", "sgz_longhun", "sgz_jiuzhu"], 
            ...
            check: function(card) {
                ...
                if (suit == 'heart') {
                    ...
                    if (att > 0) { // 如果是自己或队友
                        ...
                        if (att > 2) return (selected < 3 ? 4000 : 0);
    ...
    skillTranslate: {
        sgz_juejing: "绝境",
        sgz_juejing_info: "锁定技，①你的手牌上限+X（X为你的体力上限）；②当你进入或脱离濒死状态时摸一张牌。③回合外你的手牌数始终不小于2。④你的体力值始终不大于1，体力上限始终不大于7。⑤你单次受到的伤害不大于你已损失体力值与游戏轮数。",
        sgz_longhun: "龙魂",
        sgz_longhun_info: "你可以将1至3张花色相同的牌当做对应牌使用或打出并根据其数量与花色执行对应效果：<br>♥️当【桃】；两张：目标增加一点体力上限且回复量+1；三张：目标增加两点体力上限并回满体力。<br>♦️当火【杀】（无距离次数限制且不可被响应）；两张：伤害+1；三张：伤害改为等同于目标的体力与护甲之和且至少为3。<br>♠️当【无懈可击】：两张：获得当前回合角色一张牌；三张：不可被响应且摸两张牌。<br>♣️当【闪】：两张：获得当前回合角色一张牌；三张：弃置一名角色所有牌。<br>若你依此法使用或打出了2或3张牌，你增加一点体力上限。",
        sgz_jiuzhu: "救主",
        sgz_jiuzhu_info: "蓄力技(0/7)，每名角色准备阶段开始时或你进入濒死状态时，你获得1点蓄力点。<br>当你发动“龙魂”时，你可以消耗1点蓄力点并执行相应效果：若在你的回合内/外，你获得一名其他角色/当前回合角色的一张牌。",
    },
    characterTaici:{
