export default {
    character: {
        // 梦郭嘉：神势力，4体力
        sgz_guojia: ["male", "shen", 3, ["sgz_guanxu","sgz_kuitian","sgz_jihui","sgz_tianshang"], [
            "des:郭嘉，字奉孝，颍川奇士，曹操帐下第一谋臣。<br>建安十二年，从征乌桓，途次易州，水土不服，病势日沉。时值九月，塞外霜寒，嘉伏枕车中，形销骨立，然其双目愈发明亮，如暗夜寒星。左右劝其回军，嘉笑曰：“此正天授良机，岂可失之？”及至白狼山，敌军势大，诸将皆惧。嘉强支病体，登高望敌，忽仰天大笑，口吐鲜血，然其神思空明，若与天地共鸣。<br>当此时，嘉意气陡升，若有星辉自九霄倾泻，贯顶而入。其眸中光芒万丈，洞彻阴阳，默算天机于瞬息之间。乃以奇策授曹操：“轻骑兼程，出其不意。”操从其计，果斩蹋顿，降乌桓二十万众。是夜，嘉病势垂危，临殁前，以锦囊遗计，详陈辽东之变。操持囊泣血，嘉含笑而逝，时年三十八。世人皆见天有流星陨落，坠于易州。<br>此后数月，公孙康果斩二袁首级来献，一如嘉所料。操每临大敌，必捧锦囊而叹：“奉孝若在，不使孤至此！”赤壁败后，操犹痛哭流涕，恨嘉之早逝。然嘉虽身死，其魂魄似寄于星辰，屡次托梦献计，助操平定关西，克定汉中。及至魏室初建，追封嘉为贞侯，香火不绝。每有大事，朝堂必先祭奉孝之灵，辄有祥光应验。世人皆言，郭奉孝以凡人之躯窥破天机，虽英年早逝，然其智已化星辰，永佑魏室，千秋共仰。",
            "ext:大梦千秋/image/sgz_guojia.jpg",
            "die:ext:大梦千秋/audio/sgz_guojia/die/die.mp3",
        ]],
    },
        ...
        sgz_kuitian: {
            ...
            chooseButton: {
                dialog: function(event, player) {
                    const list = event.sgz_kuitian_list.map(name => ["锦囊", "", name]);
        ...
        sgz_guanxu: {
            ...
            clickable(player) {
                if (player.isUnderControl(true)) {
                    const cards = lib.skill.sgz_guanxu.getCards(player);
                    function createDialogWithControl(result) {
                        ...
                        if (_status.sgz_guanxu_clickable) _status.sgz_guanxu_clickable.close();
                        _status.sgz_guanxu_clickable = dialog;
                        dialog.open();
                    }
            ...
            getCards(player) {
                let cards = [];
                if (game.online) return game.requestSkillData("sgz_guanxu", "getTopCards", 10000);
                else {
            ...
            group: "sgz_guanxu_aocai",
            subSkill: {
                aocai: {
                    audio: "ext:大梦千秋/audio/sgz_guojia:1",
                    mod: {
                        cardEnabled2(card, player) {
                            if (card?.hasGaintag?.("sgz_guanxu")) {
                        ...
                        aiOrder(player, card, num) { if (card?.hasGaintag?.("sgz_guanxu")) return num + 0.1; },
                    },
                    onChooseToUse(event) {
                        if (game.online) return;
                        const player = event.player;
                        let cards = player.getCards("s", card => card.hasGaintag("sgz_guanxu"));
                        if (cards.length) game.deleteFakeCards(cards);
                        if (ui.cardPile.hasChildNodes !== false) {
                            cards = Array.from(ui.cardPile.childNodes).slice(0, player.maxHp);
                            player.directgains(game.createFakeCards(cards), null, "sgz_guanxu");
                        }
                    },
                    ...
                    async content(event, trigger, player) {
                        if (["useCard", "respond"].includes(trigger.name)) {
                            if (!trigger.skill) trigger.skill = "sgz_guanxu";
                            else await player.logSkill("sgz_guanxu");
                            const cards = await get.info("sgz_guanxu").getCards(player);
    ...
    skillTranslate: {
        sgz_kuitian: "窥天",
        sgz_kuitian_info: "持恒技，你的回合内，你可以视为使用任意普通锦囊牌。以此法使用的牌结算后，若你于本回合内发动此技能的次数大于你的体力上限，你减1点体力上限。",
        sgz_guanxu: "观虚",
        sgz_guanxu_info: "持恒技，牌堆顶的X张牌始终对你可见（X为你的体力上限）。你的回合内/外，你可以如手牌般使用或打出其中的锦囊牌/基本牌。",
        sgz_jihui: "极慧",
        sgz_jihui_info: "持恒技。出牌阶段限一次或当你每回合首次受到伤害后，若你的体力上限<10，你可重复执行以下流程：{①进行一次判定；②若出现过相同花色或你的体力上限≥10则跳出大括号里的内容；③增加一点体力上限}，你获得所有以此法产生的判定牌。",
        sgz_tianshang: "天殇",
        sgz_tianshang_info: "持恒技，锁定技。每名角色的回合结束时，若你已受伤，你将体力上限调整至与体力相同。",
    },
    characterTaici:{
        ...
        "sgz_guanxu": { order: 2, content: "不过星霭云雾，岂可挡我七窍清明。<br>星辰虽小，难掩其明。<br>此诚天赐余之主也。<br>身计国谋，不可两遂。<br>聪以知远，明以察微。<br>见微知著，识人心志。<br>天命靡常，惟德是辅。<br>以聪虑难，悉咨于上。<br>奉孝不才，愿献勤心。<br>殚思极虑，以临制变。" },
        "sgz_jihui": { order: 3, content: "人亦如星，或居空而渺然，或为彗而明夜。<br>沥血书辞，以效区区之忠。<br>借天秘力，佐公之事，感有荣焉。<br>吾计均已言明，主公可自择而行。<br>且为明公巧借天时。" },
        "sgz_tianshang":{ order: 4, content: "纵殒身祭命，亦要助明公大业！<br>行将就木，良谋难施。<br>可叹桢干命也迂。" },
